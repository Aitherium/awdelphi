...
WHY IT LIVES HERE AND NOT IN _doctor.py
---------------------------------------------------------------------------
It used to live there, and _doctor.py is GENERATED. A routine regeneration
deleted every line of it and left the --self-test flag behind, so the command
printed the ordinary doctor report and exited 0 - a PASS asserting nothing.
The generator never writes this path; _doctor.py delegates to run() when this
module exists.
"""
...
def run() -> int:
    """The seam _doctor.py calls. 0 on success, 1 on any failed check.

    Exit CODE rather than a bool, so a caller that forgets to interpret it
    still fails loudly instead of treating False as success.
    """
